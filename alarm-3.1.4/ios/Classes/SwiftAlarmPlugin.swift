...
//            result(FlutterError(code: "NATIVE_ERR", message: "[SwiftAlarmPlugin] Arguments are not in the expected format: \(call.arguments)", details: nil))
        ...
        if let audioPlayer = self.loadAudioPlayer(withURL: assetAudio, forId: id) {
            safeModifyResources {
    ...
    private func loadAudioPlayer(withURL assetAudio: String, forId id: Int) -> AVAudioPlayer? {
       let audioURL =  assetAudio
...
//        }
        ...
        do {
            return try AVAudioPlayer(contentsOf: URL(string: audioURL)!)
        } catch {
